...
ERC-3009 is the closest existing fit for this settlement pattern: no setup, a single contract call, no gas cost for the payer, and a small payload. However, ERC-3009 requires each token contract to individually implement the interface. As of 2026, only a handful of tokens support it (most notably USDC and EURC), and most deployed tokens cannot be upgraded to add it. Nor does current support close the gap for smart accounts: USDC's FiatToken v2.2 accepts smart account signatures via ERC-1271, but the check is a static call from the token, so the account's own spending policies never see the transfer, the same enforcement gap Permit2 has (see the Rationale). ERC-3009 also cannot support native asset transfers.
...
1. No enforcement point for spending policy. Permit2 settles by calling `transferFrom` on the token against a standing allowance, so the payer's account code never executes in the spend path; its only touchpoint is `isValidSignature`, a static call that cannot write state. On-chain spending policies (daily budgets, rate limits, recipient allowlists) therefore cannot be enforced, and the account's existing enforcement machinery (e.g., ERC-7579 hook modules, Safe guards) never runs. A future version cannot close this gap: Permit2 is an immutable singleton, and a successor's transfers would still execute outside the account. Under this ERC, settlement is a call on the account itself, so its policies see and can gate every spend (see Account-level policy enforcement).
...
Existing signature paths share one root cause. Where Permit2 or an ERC-3009 token (e.g., USDC's FiatToken v2.2) accepts a smart account signature at all, it consults the account through ERC-1271's `isValidSignature`, a `view` function invoked as a static call: any state write inside the account reverts. The account can answer whether a signature is valid, but it cannot record what it has approved, so it cannot keep a running budget or rate limit for its own spending. A spending limit the account cannot decrement on each spend is unenforceable.
